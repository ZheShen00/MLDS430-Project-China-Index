...
1. **Ensure prerequisites**
   - Snowflake account and network access.
   - Docker installed if you want to run Airflow locally.
...
2. **Run dbt models locally**
   ...
   ```cmd
   cd index_dbt
   set SNOWFLAKE_PRIVATE_KEY_PATH=C:\path\to\p8.key
   set SNOWFLAKE_PRIVATE_KEY_PASSPHRASE=your-passphrase
   dbt run --profiles-dir .
   dbt test --profiles-dir .
   ```

3. **Start Airflow (optional, for orchestration)**
   ...
   Then enable the index_etl_pipeline DAG from the Airflow UI (localhost:8080 by default).
   To stop the stack:
...
```text
Airflow/        # Dockerized Airflow project and ETL DAG
index_dbt/      # dbt project targeting Snowflake
streamlit/      # Streamlit analytics app
data_sample/    # Example CSV extracts (raw-like and staged)
```
...
Basic unit tests for the DAG and Fivetran trigger logic are provided in `tests/test_index_pipeline_dag.py`, checking the DAG id, task graph, and `trigger_fivetran_sync` behavior with and without environment variables. 
...
Where appropriate, dbt tests such as `not_null` and `unique` are defined in the project’s `schema.yml` files (e.g., on the `unique_key` in the staging/fact models), and basic column descriptions are included for documentation.
...
To run dbt locally on macOS/Linux:
...
`data_sample/` includes:

* `index_daily_2019_2023_filtered.csv` – raw-like daily index OHLCV data.
* `STG_INDEX_DAILY.csv` – staged output matching the `stg_index_daily` schema.

These can be used for local testing or loading into Snowflake for development.
...
* Streamlit relies on `streamlit`, `snowflake-connector-python`, `pandas`, `numpy`, and `altair`, which can be installed with `pip install -r Airflow/requirements.txt` or in a dedicated virtual environment.
