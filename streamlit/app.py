...
def show_volatility_tab(df: pd.DataFrame):
    ...
    weekday_ret = (
        df.groupby(["index_label", "weekday_name"])["daily_return"]
        .mean()
        .reset_index()
    )
    weekday_ret["daily_return_pct"] = weekday_ret["daily_return"] * 100

    weekday_pivot = weekday_ret.pivot(
        index="weekday_name", columns="index_label", values="daily_return_pct"
    )
    weekday_pivot = weekday_pivot.reindex(["Mon", "Tue", "Wed", "Thu", "Fri"])
    st.bar_chart(weekday_pivot)
